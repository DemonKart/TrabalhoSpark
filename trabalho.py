...
courses = courses.select("CO_CURSO", "NO_OCDE")
institutions = institutions.select("CO_IES", "CO_UF_IES")

course_names = courses.select("NO_OCDE").distinct().collect()
...
for course in courses2:
	courses.append((course[0],names.index(Row(NO_OCDE=course[1]))))

courses = spark.createDataFrame(courses, ["CO_CURSO", "NO_OCDE"])
...
todas = ['NU_IDADE_ALUNO', 'IN_SEXO_ALUNO', 'CO_TIPO_ESCOLA_ENS_MEDIO', 'CO_COR_RACA_ALUNO', 'CO_UF_IES', 'NO_OCDE']
...
for i in range(0,len(todas)-1):
	...
	print varx + ": ", total, missed, float(missed) / float(total)
